...
// КОНСТАНТИ ДЛЯ НАЛАШТУВАННЯ ВАЛІДАЦІЇ ТА КЕРУВАННЯ ЗАПИТІВ
const MIN_USERNAME_LENGTH = 3;
const USER_PROFILE_QUERY_KEY = "user-profile";
const AVATAR_SIZE = 120; // СТАНДАРТНИЙ РОЗМІР ДЛЯ СТОРОННЬОГО АВАТАРА КОРИСТУВАЧА

// ШЛЯХИ ТА СТАТИЧНІ РЯДКИ ДЛЯ РЕДІРЕКТУ Й ЗАГЛУШОК
const PROFILE_REDIRECT_PATH = "/profile";
const DEFAULT_USER_EMAIL = "user_email@example.com";

// СТИЛІСТИЧНІ КОНСТАНТИ З ВИКОРИСТАННЯМ СТАНДАРТНИХ ПІКСЕЛІВ ЗАМІСТЬ REM
...
// СТИЛІ ДЛЯ СТАНУ ЗАВАНТАЖЕННЯ ТА ВІДСУТНОСТІ ДАНИХ
// const LOADING_BOX_STYLES = {
//   padding: "32px",
//   color: "#ffffff",
// };

// ПОЧАТКОВІ СТАНІ ПОЛІВ ФОРМИ ЗА ЗАМОВЧУВАННЯМ
...
export default function EditProfilePage() {
  ...
  // МУТАЦІЯ ДЛЯ ВІДПРАВКИ PATCH-ЗАПИТУ НА ЗМІНУ ДАНИХ КОРИСТУВАЧА
  ...
  >({
    ...
    onSuccess: (updatedUser) => {
      ...
      // ОНОВЛЕННЯ КЕШУ ДЛЯ ПЕРЕЗАВАНТАЖЕННЯ АКТУАЛЬНИХ ДАНИХ КВЕРІ
      queryClient.invalidateQueries({ queryKey: [USER_PROFILE_QUERY_KEY] });
      router.push(PROFILE_REDIRECT_PATH);
    },
  });
  ...
  return (
    <main className={css.mainContent}>
      {/* КАРТКА-ОБГОРТКА ДЛЯ ФОРМИ РЕДАГУВАННЯ ЗА СТИЛЯМИ ПРОЄКТУ */}
      <div className={css.profileCard}>
        <h1 className={css.formTitle}>Edit Profile</h1>

        {/* ВИКОРИСТАННЯ ОПТИМІЗОВАНОГО КОМПОНЕНТА IMAGE ЗАМІСТЬ IMG */}
        {user?.avatar && (
          ...
          {({ isSubmitting }) => (
            <Form className={css.form}>
              <div className={css.profileInfo}>
                ...
                <p className={css.email}>Email: {user?.email || DEFAULT_USER_EMAIL}</p>
              </div>

              {/* БЕЗПЕЧНЕ ВИВЕДЕННЯ СЕРВЕРНОЇ ПОМИЛКИ ВІД API AXIOS ВИТЯГНУТО З ІНЛАЙН СТИЛІВ */}
              {error && axios.isAxiosError(error) && (
                <div style={SERVER_ERROR_STYLES}>
              ...
              <div className={css.actions}>
                <button
                  ...
                  onClick={() => router.push(PROFILE_REDIRECT_PATH)}
                  disabled={isPending}
                >
                  Cancel
                </button>
